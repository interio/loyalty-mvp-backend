...
{
    Task<List<RewardProduct>> GetByIdsAsync(Guid tenantId, IEnumerable<Guid> ids, CancellationToken ct = default);
}

/// <summary>Inventory operations for reward catalog.</summary>
public interface IRewardInventoryService
{
    Task ReserveAsync(Guid tenantId, Guid rewardProductId, int quantity, CancellationToken ct = default);
    Task ReleaseAsync(Guid tenantId, Guid rewardProductId, int quantity, CancellationToken ct = default);
}
